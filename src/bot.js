...
console.log('\nLoading...');
console.log('If this takes too long, please make sure you have used a correct token');
const fs = require('fs');
const yaml = require('js-yaml');
const { mainprefix, token, status } = yaml.load(fs.readFileSync('./src/config.yml'));
...
client.on('message', async (message) => {
    ...
    if (message.content.startsWith(prefix)) {
        ...
        if (premiumcheck && premiumcheck.find((find) => find.kid == message.author.id)) return message.channel.send(`You cannot use this bot as you are blacklisted!`);
...
// For checking for filtered words.
